...
int fine_tuning(Option opt)
{
    ...
    ffm_model *old_model = ffm_load_model(opt.old_model_path.c_str());
    if(old_model == nullptr)
    {
        cerr << "cannot load " << opt.old_model_path << endl << flush;
        return 1;
    }
    tr->n = old_model->n;
    tr->m = old_model->m;
    ...
    {
        ffm_model *model = ffm_fine_tuning_with_validation(tr, va, opt.param, old_model);
